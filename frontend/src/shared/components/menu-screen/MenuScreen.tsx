...
export function MenuScreen({ children, className = '', img, panelClassName = '', subtitle, title }: MenuScreenProps) {
  const screenClassNames = ['flex min-h-screen w-screen items-center justify-center px-4 py-8 text-text-light sm:p-8', className]
    .filter(Boolean)
    .join(' ');
  ...
  return (
    <Background className={screenClassNames} img={img}>
      <section className={panelClassNames}>
        ...
        <div className={['flex flex-col gap-5', title || subtitle ? 'mt-6' : ''].filter(Boolean).join(' ')}>{children}</div>
      </section>
    </Background>
  );
}
